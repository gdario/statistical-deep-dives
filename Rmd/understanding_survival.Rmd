...
We now fit a Cox proportional hazard model to this dataset. For simplicity, we will consider only two predictors, `age` and `nodes`. We will make no effort to optimize this model, which is used only for illustration purposes. The `print.coxph()` method provides a concise summary of the fit.
...
The second one difference and multiply by two, we get exactly the value of the likelihood ratio test. Rather strangely, there seems to be no slot in the `survfit.object` containing the value of the likelihood ratio test.
...
The values of `fit$wald.test` and `fit$score` is also based on the comparison of the initial and final values.

The `summary()` function some more information about these tests. One can, for example, compare the following
...
method takes a `coxph` object and, optionally, a `newdata` data frame. The `type` argument specifies the type of predicted value. Choices are:
...
It is important to note that we don't need to explicitly compute the baseline hazard in order to estimate the survival probability for a given set of predictors at a given set of time points. The output depends on whether we pass a `newdata` argument or not. According to the documentation
...
However, if we pass a `newdata` data frame to `survfit`, we obtain a survival curve *for each row* of the data frame. For example:
